# Инструкция к версии 1 (архив)

> **Это документация предыдущей реализации.** Она описывает воркфлоу
> `workflows/instagram-telegram-publisher.json`, который остаётся в репозитории и
> продолжает работать. Актуальная версия описана в корневом `README.md`.
>
> Что здесь по-прежнему верно и полезно: раздел про запуск через
> `docker-compose`, получение токенов Telegram, Gemini и Instagram, туннель
> Cloudflare и полезные команды Docker. Эти шаги общие для обеих версий.
>
> Что устарело:
>
> | В версии 1 | В версии 2 |
> |------------|------------|
> | сбор альбома паузой 7 секунд внутри воркфлоу | сбор через блокировку в Postgres, гонки исключены |
> | один воркфлоу целиком | четыре: черновик, публикация, расписание, ошибки |
> | настройки бренда внутри нод | таблица `accounts`, несколько аккаунтов Instagram |
> | промпт внутри ноды | таблица `prompt_templates`, промпт рендерится в SQL |
> | только фото и карусели | плюс видео и Reels, плюс сторис |
> | правки текста не предусмотрены | цикл правок с историей версий |
> | `GEMINI_MODEL=gemini-flash-latest` | `models/gemini-2.5-flash` — фиксированная версия, чтобы тон постов не поехал сам собой |
>
